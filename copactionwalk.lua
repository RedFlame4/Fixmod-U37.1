...
function CopActionWalk:_sanitize()
	...
	if not self._walk_anim_lengths[current_pose] then -- Our current pose lacks animations, switch our pose
				...
				if stop_pose and not self._next_is_nav_link then
					if stop_pose ~= anim_data.pose then
						self._ext_movement:action_request({
							body_part = 4,
							type = stop_pose,
							no_sync = true -- clients should handle this independently as the unit on their end is likely in a different position, CopActionCrouch/CopActionStand handle this better than a simple redirect
						})
					end
				else
...
function CopActionWalk:_upd_nav_link(t)
	...
	elseif self._simplified_path[2] or not self._persistent then
		self._simplified_path[1] = mvec3_cpy(self._common_data.pos)
		...
		if self._simplified_path[2] then
			...
			self:update(t)
		else -- this should absolutely never happen but does because of the sync issue fixed in this function
			self._expired = true
...
function CopActionWalk:_chk_correct_pose()
	...
	if not allowed_poses[pose] then
		self._ext_movement:action_request({
			body_part = 4,
			type = switch_table[pose],
			no_sync = true -- clients should handle this independently
		})
	end
