...
function CopActionCrouch:init(action_desc, common_data)
	...
	if ext_anim.move then
		...
		if ext_anim.run_start_turn then
			walk_anim_length = common_data.ext_movement._actions.walk._walk_anim_lengths.crouch[common_data.stance.name].run_start_turn[ext_anim.move_side]
		elseif ext_anim.run_start then
			walk_anim_length = common_data.ext_movement._actions.walk._walk_anim_lengths.crouch[common_data.stance.name].run_start[ext_anim.move_side]
		elseif ext_anim.run_stop then
			walk_anim_length = common_data.ext_movement._actions.walk._walk_anim_lengths.crouch[common_data.stance.name].run_stop[ext_anim.move_side]
		else
			walk_anim_length = common_data.ext_movement._actions.walk._walk_anim_lengths
			walk_anim_length = walk_anim_length and walk_anim_length["crouch"][common_data.stance.name]
	...
	if common_data.ext_movement:play_redirect("crouch", enter_t) then
